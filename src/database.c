               ...
               for (jj = 1, pmod = 1.0; jj < 41; jj++, pmod = 1.0 / (double) jj) {
                  if (fabs(theop * pmod - cand->ppsr) < (4 * cand->ppsrerr)) {
                     for (kk = 1, bmod = 1.0; kk < 10;
                        ...
                        if (fabs
                            (pulsardata[ii].orb.p * bmod - cand->pbin / SECPERDAY) <
                           ...
                           if (strlen(pulsardata[ii].bname) == 0)
                              ...
                              sprintf(psrname, "B%s", pulsardata[ii].bname);
                           if (jj > 1) {
                              sprintf(tmp1, "Possibly the %s phasemod harmonic ",
                                      num[kk]);
                              if (full) {
                                 ...
                                 sprintf(tmp3,
                                         ...
                                         psrname, theop, pulsardata[ii].orb.p);
                                 sprintf(output, "%s%s%s", tmp1, tmp2, tmp3);
                              } else {
                                 sprintf(shortout, "%s H %s", num[kk], psrname);
                                 strncpy(output, shortout, 20);
                              }
                           } else {
                              if (full) {
